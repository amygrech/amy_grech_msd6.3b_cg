	...
	public void OnMouseUp()
	{
		if (enabled)
		{
			...
			float shortestDistanceFromPieceSquared =
				(closestSquareTransform.position - thisTransform.position).sqrMagnitude;
			...
			Transform originalParent = thisTransform.parent;
			Vector3 originalPosition = originalParent.position;

			// In networked games, send move to server for validation
			if (NetworkManager.Singleton != null && NetworkManager.Singleton.IsConnectedClient)
			{
				// If we're allowed to make this move, send it to the server for validation
				if (ChessNetworkManager.Instance.CanMoveCurrentPiece(PieceColor))
				{
					// Get the BoardSynchronizer
					BoardSynchronizer boardSync = FindObjectOfType<BoardSynchronizer>();
					if (boardSync != null)
					{
						// First, give immediate visual feedback by moving the piece
						thisTransform.SetParent(closestSquareTransform);
						thisTransform.localPosition = Vector3.zero;

						// Then send move to server for validation
						// If the move is invalid, the server will tell us to move it back
						boardSync.ValidateMoveServerRpc(
						...
						Debug.Log($"Sent move from {startSquare} to {endSquare} to server for validation");
					}
					else
					{
						// Fallback if BoardSynchronizer not found - reset position
						Debug.LogWarning("BoardSynchronizer not found - resetting piece position");
				...
				else
				{
					...
					Debug.Log("Cannot move this piece - not your turn or piece");
					thisTransform.position = originalPosition;
				}
			}
			else
			{
				// In single-player mode, use the original event-based approach
